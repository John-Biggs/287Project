...
float normalizeDegrees(float degrees) {
	return (degrees >= 0) ? (degrees - (int)(degrees / 360) * 360) : (degrees + (int)(((degrees*-1) + 360) / 360) * 360);
}
...
float normalizeRadians(float rads) {
	return (rads >= 0) ? (rads - (int)(rads / M_2PI) * M_2PI) : (rads + (int)(((rads*-1) + M_2PI) / M_2PI) * M_2PI);
}
